...
class OrderRepository extends BaseRepository
{
    ...
    public function createChildOrder($id, $request): void
    {
        ...
        foreach ($productsByShop as $shop_id => $cartProduct) {
            ...
            // Логируем созданный дочерний заказ для отладки
            \Log::info('OrderRepository::createChildOrder - Дочерний заказ создан', [
                ...
                'orderInput_order_status' => $orderInput['order_status'] ?? 'не установлен',
                'orderInput_payment_status' => $orderInput['payment_status'] ?? 'не установлен',
            ]);
     ...
     * Format: 8 digits - 4 digits (e.g., 02786708-0240)
     *
     * @return string
     * @throws Exception
     */
    ...
    {
        // Генерируем формат: 8 цифр-4 цифры
        // Пример: 02786708-0240
        
        do {
            // Первая часть: 8 случайных цифр
            $firstPart = str_pad(random_int(0, 99999999), 8, '0', STR_PAD_LEFT);
            
            // Вторая часть: 4 случайные цифры
            $secondPart = str_pad(random_int(0, 9999), 4, '0', STR_PAD_LEFT);
            
            $trackingNumber = $firstPart . '-' . $secondPart;
    ...
    /**
     * Static helper method to generate unique tracking number
     * Format: 8 digits - 4 digits (e.g., 02786708-0240)
     * Can be used without repository instance
     *
     * @return string
     * @throws Exception
     */
    public static function generateTrackingNumberStatic(): string
    {
        // Генерируем формат: 8 цифр-4 цифры
        // Пример: 02786708-0240
