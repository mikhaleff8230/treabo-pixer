...
class CheckoutRepository
{
    ...
    public function verify($request)
    {
        ...
        // Цифровой маркетплейс: расчёт доставки отключён (восстановление: PHYSICAL_SHIPPING_ENABLED)
        $shipping_charge = config('services.marketplace.physical_shipping_enabled', false)
            ? (!empty($settings['options']['freeShipping']) && $settings['options']['freeShippingAmount'] <= $amount ? 0 : $this->calculateShippingCharge($request, $amount))
            : 0;
