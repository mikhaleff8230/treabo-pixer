...
class AttributeController extends CoreController
{
    ...
    public function index(Request $request)
    {
        ...
        $query = $this->repository->where('language', $language);
        
        // Фильтр по is_common, если передан
        if ($request->has('is_common')) {
            $isCommon = filter_var($request->is_common, FILTER_VALIDATE_BOOLEAN);
            $query->where('is_common', $isCommon);
        }
        
        return $query->with(['values', 'shop'])->get();
    }
    ...
    public function show(Request $request, $params)
    {
        ...
        try {
            ...
            if (is_numeric($params)) {
                $params = (int) $params;
                $attribute = $this->repository->with(['values' => function($query) use ($language) {
            ...
            $attribute = $this->repository->with(['values' => function($query) use ($language) {
                $query->where('language', $language);
            }])->where('slug', $params)->where('language', $language)->firstOrFail();
            return $attribute;
        } catch (MarvelException $e) {
            throw new MarvelException(NOT_FOUND);
        }
