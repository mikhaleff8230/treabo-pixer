...
class PlaceController extends CoreController
{
    ...
    public function feed(Request $request)
    {
        try {
            ...
            if ($request->has('hashtag_slug')) {
                ...
                // Проверяем, что slug соответствует формату
                if (!preg_match('/^[a-z0-9\-]+$/', $hashtagSlug)) {
                    $hashtagSlug = null;
                }
            }
            ...
            Log::info('PlaceController::feed - параметры', [
                'limit' => $limit,
                'cursor' => $cursor,
                'hashtag_slug_raw' => $request->get('hashtag_slug'),
                'hashtag_slug_cleaned' => $hashtagSlug,
            ]);

            Log::info('PlaceController::feed - валидация пройдена');
            ...
            return response()->json([
                'success' => true,
                'data' => PlaceFeedResource::collection($places),
                'meta' => [
                    'next_cursor' => $nextCursor,
                    'has_more' => $hasMore,
                ],
    ...
    public function show(Request $request, $slugId)
    {
        ...
        if ($parsed) {
            ...
            // Логирование для отладки хештегов
            Log::info('PlaceController::show - загрузка плейса', [
                'place_id' => $place->id,
                'hashtags_loaded' => $place->relationLoaded('hashtags'),
                'hashtags_count' => $place->hashtags ? $place->hashtags->count() : 0,
                'hashtags' => $place->hashtags ? $place->hashtags->map(function($h) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Логика идентична методу update для консистентности
            if ($request->has('hashtags')) {
                $hashtagIds = [];
                $hashtagsInput = $request->input('hashtags');
                
                if (is_array($hashtagsInput)) {
                    foreach ($hashtagsInput as $tag) {
                        // Пропускаем пустые строки (они используются для обозначения пустого массива)
                ...
                // Всегда вызываем sync, даже если массив пустой (удалит все хештеги)
                $place->hashtags()->sync($hashtagIds);
                
                Log::info('PlaceController::store - хештеги сохранены', [
                    'place_id' => $place->id,
                    'hashtags_count' => count($hashtagIds),
    ...
    public function update(Request $request, $id)
    {
        ...
        // Обрабатываем хештеги
        // ВАЖНО: если hashtags переданы (даже пустой массив), обновляем связь
        // Если hashtags не переданы вообще, оставляем существующие без изменений
        Log::info('PlaceController::update - проверка хештегов', [
            'place_id' => $place->id,
            'has_hashtags' => $request->has('hashtags'),
            'hashtags_input' => $request->input('hashtags'),
            'hashtags_all' => $request->all(),
        ]);
        ...
        if ($request->has('hashtags')) {
            ...
            Log::info('PlaceController::update - обработка хештегов', [
                'place_id' => $place->id,
                'hashtags_input' => $hashtagsInput,
                'is_array' => is_array($hashtagsInput),
            ]);
                ...
                foreach ($hashtagsInput as $tag) {
                    ...
                    if (is_string($tag) && !empty(trim($tag))) {
                        ...
                        if (!empty($tagName)) {
                            ...
                            Log::info('PlaceController::update - хештег обработан', [
                                'place_id' => $place->id,
                                'tag_name' => $tagName,
            ...
            // Всегда вызываем sync, даже если массив пустой (удалит все хештеги)
            // Если $hashtagIds пустой, sync([]) удалит все связи
            $place->hashtags()->sync($hashtagIds);
            
            Log::info('PlaceController::update - хештеги обновлены', [
                'place_id' => $place->id,
                'hashtags_count' => count($hashtagIds),
                'hashtag_ids' => $hashtagIds,
                'current_hashtags' => $place->hashtags->pluck('id')->toArray(),
            ]);
        } else {
            Log::info('PlaceController::update - хештеги не переданы, оставляем существующие', [
                'place_id' => $place->id,
                'current_hashtags' => $place->hashtags->pluck('id')->toArray(),
            ]);
        }
    ...
    {
        try {
            $place = Place::with('hashtags')->findOrFail($id);
            $hashtagIds = $place->hashtags->pluck('id')->toArray();

            // Используем оптимизированный запрос для similar places
            $query = Place::query()
                ->select([
                    'id',
                    'title',
                ...
                ->where('id', '!=', $id)
                ->whereHas('hashtags', function ($q) use ($hashtagIds) {
            ...
            $query->with([
                // Только одно превью изображение
                'images' => function ($q) {
                    $q->select(['id', 'place_id', 'url'])
                      ->orderBy('id', 'asc') // Первое загруженное изображение
                      ->limit(1);
                },
                // Минимальные данные пользователя
                'user' => function ($q) {
                    $q->select(['id', 'name'])
                      ->with('profile:id,user_id,avatar'); // Только для аватара
                },
            ])
            ...
            $limit = $request->get('limit', 12); // По умолчанию 12, как указано в требованиях
            $places = $query->limit(min($limit, 50))->get(); // Максимум 50 для similar

            Log::info('PlaceController::similar - выполнен запрос', [
                'place_id' => $id,
                'similar_places_count' => $places->count(),
                'limit' => $limit,
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Ошибка при получении похожих плейсов',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
