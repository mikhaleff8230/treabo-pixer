...
class ShopController extends CoreController
{
    ...
    public function approveShop(Request $request)
    {
        ...
        try {
            ...
            // Отправляем событие об одобрении магазина
    ...
    public function addStaff(UserCreateRequest $request)
    {
        try {
            if ($this->repository->hasPermission($request->user(), $request->shop_id)) {
                ...
                event(new StaffAdded($shop, $user, $request->user()));
    ...
    public function myShops(Request $request)
    {
        try {
            ...
            // ВОССТАНОВЛЕНА ОРИГИНАЛЬНАЯ ЛОГИКА:
            ...
            // Возвращаем магазины текущего пользователя (sale@sancan.ru)
            $shops = Shop::where('owner_id', $user->id)
                ->where('is_active', 1)
                ->get();

            \Illuminate\Support\Facades\Log::info('myShops API called', [
                'user_id' => $user->id,
                'user_email' => $user->email,
                'user_permissions' => $user->permissions->pluck('name'),
                'shops_count' => $shops->count(),
                'shops' => $shops->pluck('id', 'name'),
                'owner_id_filter' => $user->id
            ]);

            return response()->json(['data' => $shops], 200);
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('myShops API error', [
                'error' => $e->getMessage(),
