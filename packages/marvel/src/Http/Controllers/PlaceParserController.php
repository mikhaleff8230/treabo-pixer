        ...
        try {
            ...
            foreach ($places as $index => $placeData) {
                try {
                    ...
                    $imagePath = $placeData['image'];
                    if (!empty($imagePath)) {
                        // Конвертируем серверный путь в путь относительно public папки
                        if (str_starts_with($imagePath, '/var/www/sancan.ru/')) {
                            $imagePath = str_replace('/var/www/sancan.ru/', '/var/www/sancan.ru/pixer-api/public/', $imagePath);
                        }
                        ...
                        try {
                            ...
                            Log::warning('PlaceParserController::createBulk - ошибка загрузки изображения', [
                                'place_index' => $index,
                                'original_path' => $placeData['image'],
                                'converted_path' => $imagePath,
                                'error' => $e->getMessage(),
                            ]);
                        }
                        } // закрываем if (file_exists($imagePath))
                    } // закрываем if (!empty($imagePath))
