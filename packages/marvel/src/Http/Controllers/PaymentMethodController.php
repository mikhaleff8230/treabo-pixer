...
class PaymentMethodController extends CoreController
{
    ...
    {
        // Get payment gateway from request or default to stripe
        $gateway = $request->get('payment_gateway', 'stripe');
        $user = $request->user();
        return $this->repository->with('payment_gateways')->whereRelation('payment_gateways', 'user_id', $user->id)->whereRelation('payment_gateways', 'gateway_name', $gateway)->get();
    }
    ...
    public function savePaymentMethod(Request $request)
    {
        ...
        switch ($gateway) {
            case 'STRIPE':
                ...
                break;
            case 'TINKOFF':
                return $this->repository->saveTinkoffCard($request);
                break;
        }
    }
    ...
    {
        $gateway = strtoupper($this->settings->options['paymentGateway']);
        switch ($gateway) {
            case 'STRIPE':
                $setupIntent = $this->repository->setStripeIntent($request);
                break;
            case 'TINKOFF':
                $setupIntent = $this->repository->setTinkoffIntent($request);
                break;
        }
