...
protected function extractYandexProduct($offer, $fieldMapping)
{
    ...
    // Собираем все <picture> для галереи
    $pictures = [];
    ...
    if (!empty($pictures)) {
        $product['gallery'] = $pictures;           // галерея как массив
        $product['image'] = (string)$pictures[0];  // главное изображение
    } else {
        $product['gallery'] = [];
        $product['image'] = null;
    }
...
foreach ($parts as $url) {
    ...
    [$origUrl, $thumbUrl] = $this->storeRemoteWithThumb($url, $options);

    // Берем только реально загруженные URL
    if ($origUrl) {
        $stored[] = [
