...
trait PaymentStatusManagerWithOrderTrait
{
    ...
    public function stripe($order, $request, $settings): void
    {
        try {
            ...
            if (isset($order->payment_intent)) {
                foreach ($order->payment_intent as $key => $intent) {
                    if (strtoupper($request['payment_gateway']) === strtoupper($order->payment_gateway)) {
                        $chosen_intent = $intent;
                    }
                }
            }
 ...
 /**
     * Status change for tinkoff
     *
     * @throws Exception
     */
    public function tinkoff(Order $order, Request $request, Settings $settings): void
    {
        try {
            $chosen_intent = '';
...
/**
     * Status change for intellectmoney
     *
     * @throws Exception
     */
    public function intellectmoney(Order $order, Request $request, Settings $settings): void
    {
        try {
            $chosen_intent = '';
    ...
    public function mollie(Order $order, Request $request, Settings $settings): void
    {
        try {
            ...
            if (isset($order->payment_intent)) {
                foreach ($order->payment_intent as $key => $intent) {
                    if (ucfirst(strtoupper($request['payment_gateway'])) === strtoupper($intent->payment_gateway)) {
                        $chosen_intent = $intent;
                    }
                }
            }
    ...
    /**
     * Status change for yookassa
     *
     * @throws Exception
     */
    public function yookassa(Order $order, Request $request, Settings $settings): void
    {
        try {
            $chosen_intent = '';
