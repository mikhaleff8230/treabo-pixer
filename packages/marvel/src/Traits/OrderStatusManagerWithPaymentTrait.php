...
trait OrderStatusManagerWithPaymentTrait
{
    ...
    {
        // Если заказ отменен, вызываем событие для всех статусов платежа
        ...
        if ($payment_status === PaymentStatus::PENDING) {
            ...
            switch ($order_status) {
                ...
                case 'order-completed':
                    ...
                    $this->manageVendorBalance($order, $order->order_status, $order->order_status);
                    break;
    ...
    {
        \Log::info('OrderStatusManagerWithPaymentTrait::orderStatusManagementOnCancelled - Начало обработки отмены', [
            'order_id' => $order->id,
        ...
        if ($order->parent_id) {
            // Это дочерний заказ - обновляем родительский
            $parent_order = Order::find($order->parent_id);
            if (!$parent_order) {
                \Log::error('OrderStatusManagerWithPaymentTrait::orderStatusManagementOnCancelled - Родительский заказ не найден', [
                    'order_id' => $order->id,
                    'parent_id' => $order->parent_id,
                ]);
                // Продолжаем обработку только текущего заказа
                $order->cancelled_amount = $order->total ?? 0;
                $order->paid_total = 0;
                $order->total = 0;
                $order->save();
                return;
            ...
            // Это родительский заказ
            $parent_order = $order;
        }

        $tax_amount = $parent_order->sales_tax ?? 0;
        $delivery_fee = $parent_order->delivery_fee ?? 0;
        $currently_paid = $parent_order->paid_total ?? 0;
        ...
        if ($order->parent_id) {
            \Log::info('OrderStatusManagerWithPaymentTrait::orderStatusManagementOnCancelled - Обработка дочернего заказа', [
                'order_id' => $order->id,
                'parent_id' => $order->parent_id,
            ]);

            $reducedRevenueAmount = $amount - ($order->amount ?? 0);
            $cancelledTaxAmount = (($order->amount ?? 0) * $tax_rate) / 1000000;
            $reducedTaxAmount = $parent_order->sales_tax - $cancelledTaxAmount; //for precision

            $parent_order->sales_tax = max(0, $reducedTaxAmount);
            $parent_order->cancelled_tax = ($parent_order->cancelled_tax ?? 0) + $cancelledTaxAmount;

            $parent_order->paid_total = max(0, $reducedRevenueAmount + $reducedTaxAmount + $delivery_fee);
            $parent_order->total = max(0, $reducedRevenueAmount + $reducedTaxAmount + $delivery_fee);
            $parent_order->cancelled_amount = ($parent_order->cancelled_amount ?? 0) + ($order->amount ?? 0) + (($order->amount ?? 0) * $tax_rate) / 1000000;
            $parent_order->save();
            ...
            // Это родительский заказ - обрабатываем все дочерние
            \Log::info('OrderStatusManagerWithPaymentTrait::orderStatusManagementOnCancelled - Обработка родительского заказа', [
                'order_id' => $order->id,
            ]);

            // Убеждаемся, что children загружены
            if (!$parent_order->relationLoaded('children')) {
                $parent_order->load('children');
            }
            ...
            if ($childOrders && $childOrders->count() > 0) {
                foreach ($childOrders as $childOrder) {
                    ...
                    $childOrder->cancelled_amount = $childOrder->total ?? 0;
            ...
            $parent_order->cancelled_amount = ($parent_order->cancelled_amount ?? 0) + ($parent_order->paid_total ?? 0);
            $parent_order->cancelled_tax = ($parent_order->cancelled_tax ?? 0) + ($parent_order->sales_tax ?? 0);
            $parent_order->cancelled_delivery_fee = $parent_order->delivery_fee ?? 0;
            $parent_order->sales_tax = 0;
            $parent_order->delivery_fee = 0;
        ...
        \Log::info('OrderStatusManagerWithPaymentTrait::orderStatusManagementOnCancelled - Обработка завершена', [
            'order_id' => $order->id,
            'tracking_number' => $order->tracking_number,
        ]);
    }
