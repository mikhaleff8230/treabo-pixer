...
trait PaymentTrait
{
    ...
    public function processPaymentIntent($request, Settings $settings): object
    {
        try {
            ...
            $requested_payment_gateway = strtoupper($request['payment_gateway']);
            $order = $this->fetchOrderByTrackingNumber($order_tracking_number);
            $initial_payment_gateway = strtoupper($order->payment_gateway);

            if ($requested_payment_gateway !== $initial_payment_gateway) {
                $chosen_payment_gateway = strtoupper(strtolower($request['payment_gateway']));
            } else {
                if (isset($settings->options['paymentGateway'])) {
                    foreach ($settings->options['paymentGateway'] as $key => $available_gateway) {
                        if (strtoupper($available_gateway['name']) === $requested_payment_gateway) {
                            $chosen_payment_gateway = strtoupper($available_gateway['name']);
                        }
                    }
                }
            }
            ...
            if (isset($chosen_payment_gateway) && !empty($chosen_payment_gateway)) {
                ...
                if (!$is_payment_intent_exists) {
                    ...
                    if ($request['recall_gateway'] && isset($newPaymentIntent)) {
                        $this->deleteOlderPaymentIntent($order_tracking_number, strtoupper(strtolower($order->payment_gateway)));
            ...
            $result = PaymentIntent::where(function ($query) use ($order_tracking_number) {
                $query->where('tracking_number', '=', $order_tracking_number)
                    ->orWhere('order_id', '=', $order_tracking_number);
            })->where('payment_gateway', '=', strtoupper($chosen_payment_gateway))->first();
            if (!$result) {
                return (object)[
                    'error' => true,
                    'message' => 'PaymentIntent not found',
                    'order_tracking_number' => $order_tracking_number,
                    'payment_gateway' => $chosen_payment_gateway
                ];
            }
            return $result;
        } catch (\Throwable $e) {
    ...
    public function paymentIntentExists(string $order_tracking_number, string $payment_gateway): bool
    {
        return PaymentIntent::where(function ($query) use ($order_tracking_number) {
            $query->where('tracking_number', '=', $order_tracking_number)
                ->orWhere('order_id', '=', $order_tracking_number);
        })->where('payment_gateway', '=', strtoupper($payment_gateway))->exists();
    }
    ...
    public function deleteOlderPaymentIntent(string $order_tracking_number, string $payment_gateway)
    {
        return PaymentIntent::where(function ($query) use ($order_tracking_number) {
            $query->where('tracking_number', '=', $order_tracking_number)
                ->orWhere('order_id', '=', $order_tracking_number);
        })->where('payment_gateway', '=', strtoupper($payment_gateway))->forceDelete();
    }
    ...
    public function webhookSuccessResponse($order, $order_status, $payment_status)
    {
        ...
        if ($payment_status === PaymentStatus::SUCCESS) {
            $this->ensurePurchasesForOrder($order);
            $this->ensureDigitalFilesForOrder($order);
        }
        
        // Загружаем дочерние заказы, если они не загружены
        if (!$order->relationLoaded('children')) {
            $order->load('children');
        }
        
        // Обновляем статусы дочерних заказов
        if ($order->children && $order->children->count() > 0) {
            \Log::info('PaymentTrait::webhookSuccessResponse - Обновляем дочерние заказы', [
            ...
            foreach ($order->children as $child_order) {
                ...
                if ($payment_status === PaymentStatus::SUCCESS) {
                    $this->ensurePurchasesForOrder($child_order);
                    $this->ensureDigitalFilesForOrder($child_order);
                }
                
                \Log::info('PaymentTrait::webhookSuccessResponse - Дочерний заказ обновлен', [
                    'child_order_id' => $child_order->id,
                    'child_tracking_number' => $child_order->tracking_number,
                    'shop_id' => $child_order->shop_id
                ]);
            }
        }
    }
    ...
    /**
     * Для кастомного сценария оплаты создаём доступы к цифровым файлам после успешной оплаты.
     * Метод идемпотентный: не дублирует already-created записи OrderedFile.
     */
    private function ensureDigitalFilesForOrder(Order $order): void
    {
        ...
        foreach ($order->products as $orderedProduct) {
            $quantity = (int) ($orderedProduct->pivot->order_quantity ?? 1);
            $variationOptionId = $orderedProduct->pivot->variation_option_id ?? null;
            $digitalFileId = null;
            ...
            if ($variationOptionId) {
                ...
                if ($variation && $variation->is_digital && $variation->digital_file) {
                    $digitalFileId = $variation->digital_file->id;
                }
            } else {
                $product = Product::with('digital_file')->find($orderedProduct->id);
                if ($product && $product->is_digital && $product->digital_file) {
            ...
            $existingCount = OrderedFile::where('tracking_number', $order->tracking_number)
                ->where('customer_id', $order->customer_id)
                ->where('digital_file_id', $digitalFileId)
                ->count();

            $missingCount = max(0, $quantity - $existingCount);
    ...
    private function ensurePurchasesForOrder(Order $order): void
    {
        if (!$order->customer_id) {
            return;
        }
