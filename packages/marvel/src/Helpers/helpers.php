    ...
    if (!function_exists('globalSlugify')) {
        ...
        function globalSlugify(string $slugText, string $model, string $key = '', string $divider = '-'): string
        {
            try {
                ...
                // Для Product учитываем что код хранится отдельно в slug_numeric_code
                // ВАЖНО: globalSlugify НЕ должен использоваться для Product - используйте ProductSlugService!
                // Оставляем для обратной совместимости, но всегда генерируем 12-значный код
                $slugCount = 0;
                if ($model === \Marvel\Database\Models\Product::class) {
                    // Для Product всегда генерируем 12-значный код (не используем slugCount)
                    // Это временная мера для обратной совместимости
                    // ВАЖНО: Используйте ProductSlugService для новых товаров!
                    // Устанавливаем slugCount > 0 чтобы всегда генерировался код
                    $slugCount = 1;
                } else {
                    // Для других моделей используем стандартную проверку
                    if ($key) {
