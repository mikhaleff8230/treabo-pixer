...
function createPayment($amount) {
    try {
        ...
        logDebug("Платеж успешно создан", [
...
function checkPaymentStatus($paymentId) {
    try {
        ...
        if (empty($paymentId)) {
            throw new Exception("ID платежа не может быть пустым");
        }
        
        // Очищаем ID платежа от возможных пробелов
        $paymentId = trim($paymentId);
        
        logDebug("Начало проверки статуса", [
            'raw_payment_id' => $paymentId,
            'payment_id_length' => strlen($paymentId),
        ...
        if (empty($terminalKey) || empty($password)) {
            throw new Exception("Отсутствуют необходимые параметры для запроса (TerminalKey или Password)");
        }
        ...
        logDebug("Данные запроса проверки статуса", [
        ...
        if (!isset($result['Success']) || $result['Success'] !== true) {
            ...
            if (isset($result['ErrorCode'])) {
                ...
                switch ($result['ErrorCode']) {
                    case '204':
                        $errorMessage .= "\nПлатеж не найден. Проверьте правильность ID платежа.";
                        break;
                    case '1':
                        $errorMessage .= "\nНеверный формат данных";
                        break;
                    case '2':
                        $errorMessage .= "\nНеверный токен. Проверьте правильность TerminalKey и Password.";
                        break;
                    case '3':
                        $errorMessage .= "\nНеверный терминальный ключ";
                        break;
...
<body class="bg-light">
    ...
    <script>
        ...
        document.getElementById('statusForm').addEventListener('submit', async (e) => {
            e.preventDefault();
            const paymentId = e.target.payment_id.value.trim();
            const resultDiv = document.getElementById('statusResult');
            
            if (!paymentId) {
                resultDiv.innerHTML = `
                    <div class="alert alert-danger">
                        <p>Пожалуйста, введите ID платежа</p>
                    </div>
                `;
                return;
            }
                ...
                if (data.success) {
                    ...
                    resultDiv.innerHTML = `
                        <div class="alert alert-danger">
                            <p>Ошибка: ${data.message}</p>
                            <p>ID платежа: ${paymentId}</p>
                            <p class="mt-2">Убедитесь, что:</p>
                            <ul>
                                <li>ID платежа скопирован полностью</li>
                                <li>Платеж был создан недавно</li>
                                <li>Вы используете правильный терминальный ключ</li>
                            </ul>
                        </div>
                    `;
                }
